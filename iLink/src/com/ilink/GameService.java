...
public class GameService {
	...
	public GameService(Context con,AppData ad)
	{
		context = con;
		this.ad = ad;
	}
	public void start(GradeEnum ge,ArrangeEnum ae)
	{
		board = null;
		Board board2 = new Board(context,ad);
		this.pieces = board2.init(ge, ae);
		board = board2;
	}

	public void downUpdate(OrienEnum mode)
	{
		board.downUpdate(pieces, mode);
	}
	
	public void shuffle(OrienEnum mode)
	{
		pieces = board.shuffle(pieces, mode);
	}
	...
	public Piece findPiece(float touchX, float touchY)
	{
		...
		int relativeX = (int) touchX - ad.beginCoorX;
		int relativeY = (int) touchY - ad.beginCoorY;
		// 如果鼠标点击的地方比board中第一张图片的开始x座标和开始y座标要小, 即没有找到相应的方块
		if (relativeX < 0 || relativeY < 0)
		{
			return null;
		}
		int indexX = getIndex(relativeX, ad.imageWidth);
		int indexY = getIndex(relativeY, ad.imageHeight);
		if (indexX < 0 || indexY < 0)
		{
			return null;
		}
		if (indexX >= ad.countX || indexY >= ad.countY )
		{
			return null;
		}
	...
	public LinkInfo link(Piece p1, Piece p2)
	{
		...
		if (p2.getIndexX() < p1.getIndexX())
			return link(p2, p1);
		// 获取p1的中心点
		Point p1Point = p1.getCenter(ad.imageWidth,ad.imageHeight);
		// 获取p2的中心点
		Point p2Point = p2.getCenter(ad.imageWidth,ad.imageHeight);
		// 如果两个Piece在同一行
		if (p1.getIndexY() == p2.getIndexY())
		{
			// 它们在同一行并可以相连
			if (!isXBlock(p1Point, p2Point, ad.imageWidth))
			{
		...
		{
			if (!isYBlock(p1Point, p2Point, ad.imageHeight ))
			{
				// 它们之间没有真接障碍, 没有转折点
				return new LinkInfo(p1Point, p2Point);
			}
		...
		Point cornerPoint = getCornerPoint(p1Point,p2Point,ad.imageWidth,ad.imageHeight);
		if (cornerPoint != null)
		{
		...
		Map<Point, Point> turns = getLinkPoints(p1Point,p2Point,ad.imageWidth,ad.imageHeight);
		if (turns.size() != 0)
		{
		...
		int pieceWidth, int pieceHeight)
	{
		...
		// 获取Board的最大高度
		int heightMax = (ad.countY + 1) * pieceHeight + ad.beginCoorY;
		// 获取Board的最大宽度
		int widthMax = (ad.countX + 1) * pieceWidth+ ad.beginCoorX;
		// 先确定两个点的关系
		...
		if (point1.y == point2.y)
		{
			...
			// 向上遍历
			// 以p1的中心点向上遍历获取点集合
			p1UpChanel = getUpChanel(point1, -pieceHeight, pieceHeight);
			// 以p2的中心点向上遍历获取点集合
			p2UpChanel = getUpChanel(point2, -pieceHeight, pieceHeight);
			Map<Point, Point> upLinkPoints = getXLinkPoints(p1UpChanel,
				p2UpChanel, pieceHeight);
			// 向下遍历, 不超过Board(有方块的地方)的边框
			// 以p1中心点向下遍历获取点集合
			p1DownChanel = getDownChanel(point1, heightMax+pieceHeight, pieceHeight);
			// 以p2中心点向下遍历获取点集合
			p2DownChanel = getDownChanel(point2, heightMax+pieceHeight, pieceHeight);
		...
		if (point1.x == point2.x)
		{
			...
			// 向左遍历
			// 以p1的中心点向左遍历获取点集合
			List<Point> p1LeftChanel = getLeftChanel(point1, -pieceHeight, pieceWidth);
			// 以p2的中心点向左遍历获取点集合
			p2LeftChanel = getLeftChanel(point2, -pieceHeight, pieceWidth);
			Map<Point, Point> leftLinkPoints = getYLinkPoints(p1LeftChanel,
				p2LeftChanel, pieceWidth);
			// 向右遍历, 不得超过Board的边框（有方块的地方）
			// 以p1的中心点向右遍历获取点集合
			p1RightChanel = getRightChanel(point1, widthMax+pieceHeight, pieceWidth);
			// 以p2的中心点向右遍历获取点集合
			List<Point> p2RightChanel = getRightChanel(point2, widthMax+pieceHeight,
				pieceWidth);
